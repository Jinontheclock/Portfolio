...
- **TinyPaws 케이스 스터디 v2(제작 서사)** 배포됨(`94b9b2d`): 7챕터 = The Brief/Understanding Adopters/Structuring the Site/Building the Brand/Designing the Experience/Shipping It Myself/Reflection. Problem 삼단·↔ 태그·stats 없음. cta 2곳(01·06)은 지시서상 disabled 외부 링크지만 오너의 이전 요청대로 **임베드 모달 유지**(href «TBD» 토큰은 데이터에 보존 — 배포 URL 확정 시 외부 링크 전환 가능). journey 다이어그램 삭제됨.
  - 03 Structuring 이미지 완료(`107457d`~`96cd5da`): sitemap·lo-fi 그리드 + before/after **3쌍**(nav/structure/**cta 신규**) 렌더. 오너 업로드 5개(sitemap/lo-fi/before nav·info·landing)→webp, after 3개 데모 캡처. BeforeAfter에 optional beforeAlt/afterAlt + caption 생략 지원 추가(ProLog 무회귀 검증). structure 캡션 CTA 중복 제거→"scan in seconds". **남은 미등록 figure 4종**: brand-explore / styletile / **tokens(스타일타일 크롭+tokens.css 코드 투샷)** / devspecs — 오너 export 대기.
  ...
  - 고양이 24마리로 확장(`002f6b7`): 오너가 `site/public/media/tinypaws/`에 올린 사진 16장 반영 — 신규 13(miso·clover·ginger·peanut·waffles·poppy·marmalade(사진2장)·truffle·sunny·willow·hazel·ash·pebble), clementine·tofu 일러스트 폴백 → 실사진 전환(tofu breed는 White & Grey로 수정), Miso는 nori.json 서사에 맞춰 sister·bonded pending. adopt 나이 필터가 weeks/years 파싱하도록 수정(기존 parseInt는 "3 years"를 kitten으로 오분류). 원본 업로드 파일은 site/public/media/tinypaws/에 있었으나 `5b48ef4`에서 정리(mp4·.gitkeep만 유지) — 파생 webp가 src/assets에 있으므로 원본 불필요. 새 세션에서 소스 원본이 필요하면 오너 재업로드.
  ...
  - 폴리시 패스(`655a29f`): 콜라주 피스 확대+전 피스 프레임(필에 오프화이트 링 추가, 가림 최소화 재배치, 캡션 z-순서), Featured 캣 서클 380px+카피 padding-left로 히어로와 좌우 밸런스, About "How the rescue actually runs" 660px 센터(문단 measure 정렬), 캣 프로필 둘째 사진을 본문 하단으로 이동(우측 레일 공백 해소), 인풋 전 사이트 통일(13px/16px·rgba 보더·radius-input·focus 오렌지 — donate/footer/서치 포함), co-thumb radius-input, wish-rows 갭 space-4, [data-reveal]을 html.js 게이트(무JS 안전).
  - 푸터 전화번호 문구 제거(`a368111`).
...
- TinyPaws 데모웹: GitHub `Jinontheclock/TinyPaws` main = `97d2a6c` (로컬 /home/claude/TinyPaws-web).
